...
### 1.6 👤 Granting an agent SSH access to run this runbook

An AI agent asked to execute the deployment steps below has no interactive
credentials of its own — it cannot type a password, and it starts with no
key the server already trusts. It needs its own SSH key added to
`authorized_keys` on the server before section 2 onward can run.

Procedure, each time an agent needs this:

1. The agent generates a fresh keypair local to its own environment —
   `ssh-keygen -t ed25519 -f ~/.ssh/<name>_ed25519 -N ""` — and prints the
   **public** key in chat. The private key never leaves the agent's machine
   and is never pasted anywhere.
2. You append that public key to `authorized_keys` for whichever account
   actually runs `docker` on the server:
   ```bash
   echo "<the public key line the agent printed>" >> ~/.ssh/authorized_keys
   ```
3. Tell the agent which user to connect as, so it can verify. **This has
   been a live source of confusion**: this runbook's own examples use
   `ssh root@37.32.27.201` (section 2 onward), but in a live session the
   correct login turned out to be under active dispute — neither `root@`
   nor `ubuntu@` accepted the freshly-added key on the first try. Confirm
   the actual admin account before assuming either name, and confirm the
   account can reach docker (`sudo -n docker ps` or equivalent) rather than
   assuming SSH access implies deploy access.
4. Treat this key as temporary. It is tied to one agent session, not a
   standing credential — there is no cleanup step in this runbook because
   removing stale keys from `authorized_keys` is itself a step only a human
   should do, on their own schedule.

Do **not** commit a specific public key value into this file — a key
generated by one agent session is meaningless to the next. Keep this
section procedural.
