...
- محصول فنی v1 در ریپو تقریباً آماده است (جدول قیمت و کارمزد، صفحات پلتفرم/دارایی، بلاگ، اسلات تبلیغ، `/go/`، SEO پایه).
...
- امروز همهٔ `referral_url`ها در `collector/src/tablo_collector/platforms.py` برابر `None`اند؛ `/go/<slug>` فعلاً به `website_url` می‌رود.

### چهار خروجی اجباری روز ۳۰

1. `tablo.gold` پایدار پشت آروان — معیارهای پذیرش [ops/RUNBOOK.md](../../ops/RUNBOOK.md)
...
- [ ] تنظیم CDN آروان روی `tablo.gold`
...
فایل: `collector/src/tablo_collector/platforms.py`
...
2. [ ] اختلاف ریالی خرید ۱۰۰ میلیون تومان: ارزان‌ترین vs گران‌ترین رفت‌وبرگشت
...
- [ ] اعداد فقط از slot/دادهٔ collector پر شوند (خط لولهٔ `tablo-generate` / دروازهٔ اعتبارسنجی)
...
- [ ] ساخت/ورود property برای `https://tablo.gold`
- [ ] تأیید مالکیت (DNS TXT یا روش در دسترس)
- [ ] ارسال sitemap: `https://tablo.gold/sitemap.xml`
- [ ] درخواست ایندکس برای `/` و ۲–۳ صفحهٔ پلتفرم پرترافیک‌شونده
- [ ] یادداشت نتیجهٔ تست دسترسی (معماری بند ۱۳.۱۴)

### روز ۹ — ممیزی SEO فنی

- [ ] `https://tablo.gold/robots.txt` — خط Sitemap و Disallowهای درست (`/go/` نباید ایندکس شود)
- [ ] `sitemap.xml` شامل خانه، پلتفرم‌های listed، دارایی‌های واجد شرط، بلاگ
- [ ] View-source خانه: جدول قیمت و کارمزد در HTML اولیه باشد (نه فقط بعد از JS)
...
- [ ] آماده‌سازی توضیح سایت: مقایسهٔ قیمت و کارمزد طلای آب‌شدهٔ آنلاین؛ مخاطب در قصد خرید
...
- [ ] انتشار صفحه/پست «هزینه‌ی رفت‌وبرگشت چگونه حساب می‌شود؟» (فرمول معماری بند ۱ + منبع کارمزد)
...
- [ ] اگر ~۴ هفته دادهٔ تاریخچه داری: ۱ پست روند اختلاف کارمزد
...
| اتهام thin affiliate / بی‌اعتمادی | قیمت و کارمزد در HTML + روش‌شناسی + برچسب تبلیغ + عدم فروش «پیشنهاد سردبیر» |
...
```markdown
# مدیاکیت تابلو — نسخه X.Y — تاریخ:

## یک خط
تابلو قیمت و کارمزد خرید/فروش طلای آب‌شده را بین پلتفرم‌های آنلاین ایران مقایسه می‌کند.
...
## تمایز
پوشش وسیع‌تر از رقیب مستقیم، وضعیت باز/بستهٔ معامله، برچسب دفتر سفارش، کارمزد جدا و صریح کنار قیمت.
...
```
موضوع: جایگاه آزمایشی در جدول مقایسه قیمت و کارمزد طلا — تابلو

سلام،
تابلو قیمت و کارمزد پلتفرم‌های طلای آنلاین را کنار هم مقایسه می‌کند.
...
| رجیستری سکو / referral | [collector/src/tablo_collector/platforms.py](../../collector/src/tablo_collector/platforms.py) |
