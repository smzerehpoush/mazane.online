...
/**
 * کدام سطر تجمیع خوانده شد — امروز فقط یکی هست.
 *
 * `PRICE` تنها سمت موجود است (سند تصمیم ۰۰۰۲). تفکیک `MEAN`/`MID` و ترجیح
 * بینشان حذف شد: مهاجرت ۰۱۷ سطرهای تاریخی را هم به همین یک نام رساند، پس
 * fallback دیگر چیزی برای برگشتن ندارد.
 */
