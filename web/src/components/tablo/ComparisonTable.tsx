/**
 * جدول مقایسه — همه‌ی سکوهای فهرست گردآورنده، پنج ستون (تصمیم مالک
 * ۲۰۲۶-۰۸-۱۰؛ سند تصمیم ۰۰۰۲): نام سکو، قیمت، کارمزد خرید، کارمزد فروش،
 * دکمه‌ی رفتن به سایت.
 *
 * ⚠️ هیچ محاسبه‌ی قیمتی اینجا نیست: «قیمت» عدد اعلامی خود سکوست پیش از
 * کارمزد، و کارمزدها جدا از گردآورنده می‌آیند — هیچ لایه‌ای آن‌ها را در هم
 * ضرب نمی‌کند.
 *
 * ⚠️ ستون قیمت **آنچه می‌پردازید نیست**: ترتیب ردیف‌ها هم روی همین عدد
 * پیش-از-کارمزد است، پس سکویی با کارمزد بالا می‌تواند بالاتر بنشیند. دو
 * ستون کارمزد تنها چیزی‌اند که این را برای کاربر آشکار می‌کنند — حذف یا
 * پنهان‌کردنشان جدول را گمراه‌کننده می‌کند.
 *
 * ⚠️ کارمزد «—» یعنی سکو اعلامش نکرده، **نه** اینکه صفر است. صفر عدد
 * معتبری است (داریک) و جداگانه نمایش داده می‌شود.
 *
...
import { formatMinutesAgoFa, formatPercentPointsFa, isStale, minutesSince } from "@/lib/format";
    ...
    <section className="glass-surface overflow-hidden">
      <div className="border-b border-border/70 px-4 py-4 sm:px-6">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          مرتب‌شده از ارزان‌ترین قیمت — کارمزد جداگانه در دو ستون بعدی
        </p>
      </div>
            ...
            <tbody>
              {rows.map((row) => (
                <tr
                  ...
                  {...(row.isCheapest ? { "data-cheapest": "true" } : {})}
                ...
                >
                  ...
                  <td
                    data-fee="buy"
                    className="px-3 py-3 text-xs tabular-nums text-muted-foreground sm:text-sm"
                  >
                    {row.buyFeePercent === null
                      ? "—"
                      : formatPercentPointsFa(row.buyFeePercent)}
                  </td>
                  <td
                    data-fee="sell"
                    className="px-3 py-3 text-xs tabular-nums text-muted-foreground sm:text-sm"
                  >
                    {row.sellFeePercent === null
                      ? "—"
                      : formatPercentPointsFa(row.sellFeePercent)}
                  </td>
                  <td className="px-4 py-3 sm:px-6">
                    <a
                      href={`/go/${row.slug}`}
